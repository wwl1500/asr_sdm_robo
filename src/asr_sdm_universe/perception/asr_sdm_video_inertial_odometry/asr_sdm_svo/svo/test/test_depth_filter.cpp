...
void DepthFilterTest::depthFilterCb(svo::Point * point, double depth_sigma2)
{
  ...
  double error =
    fabs(depth - depth_ref_.at<float>(point->obs_.front()->px[1], point->obs_.front()->px[0]));
  results_.push_back(
    ConvergedSeed(point->obs_.front()->px[0], point->obs_.front()->px[1], depth, error));
...
void DepthFilterTest::testReconstruction(std::string dataset_dir, std::string experiment_name)
{
  ...
  svo::DepthFilter::callback_t depth_filter_cb =
    boost::bind(&DepthFilterTest::depthFilterCb, this, _1, _2);
  ...
  for (int i = 0; it != sequence.end() && i < 20; ++it, ++i) {
    ...
    if (i == 0) {
      ...
      vk::blender_utils::loadBlenderDepthmap(
        dataset_dir + "/depth/" + (*it).image_name_ + "_0.depth", *cam_, depth_ref_);
      continue;
    }
  ...
  for (std::list<ConvergedSeed>::iterator i = results_.begin(); i != results_.end(); ++i) {
    ...
    ofs << p[0] << " " << p[1] << " " << p[2] << " " << (int)c[0] << " " << (int)c[1] << " "
        << (int)c[2] << std::endl;
  }
}
