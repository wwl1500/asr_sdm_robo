...
public:
  ...
  void testSequence(
    const std::string & dataset_dir, const std::string & experiment_name,
...
void SparseImgAlignTest::testSequence(
  ...
  svo::feature_detection::AbstractDetector * feature_detector)
{
  vk::FileReader<vk::blender_utils::file_format::ImageNameAndPose> sequence_file_reader(
    dataset_dir + "/trajectory.txt");
  std::vector<vk::blender_utils::file_format::ImageNameAndPose> sequence;
  sequence_file_reader.skipComments();
  if (!sequence_file_reader.next()) std::runtime_error("Failed to open sequence file");
  ...
  std::string trace_name(
    trace_dir + "/sparse_img_align_" + experiment_name + "_trans_estimate.txt");
  ...
  for (int i = 0; iter != sequence.end() && i < 30; ++iter, ++i) {
    ...
    if (i == 0) {
      ...
      feature_detector->detect(
        frame_ref_.get(), frame_ref_->img_pyr_, svo::Config::triangMinCornerScore(),
        frame_ref_->fts_);
    ...
    // run image align
    vk::Timer t;
    svo::SparseImgAlign img_align(
      svo::Config::kltMaxLevel(), svo::Config::kltMinLevel(), 30, svo::SparseImgAlign::GaussNewton,
      false, false);
    ...
    printf(
      "[%3.i] time = %f ms \t |t| = %f \t translation error = %f\n", i, t.stop() * 1000,
      (frame_ref_->T_f_w_ * T_gt_w.inverse()).translation().norm(), translation_error.back());
  ...
  for (std::list<double>::iterator it = translation_error.begin(); it != translation_error.end();
       ++it)
    ofs << *it << std::endl;
  ofs.close();
}
