...
public:
  MatcherTest()
  {
    ...
    vk::blender_utils::loadBlenderDepthmap(
      dataset_dir + "/depth/frame_000002_0.depth", *cam_, depth_ref_);
  }
...
void MatcherTest::testEpipolarSearchFullImg()
{
  ...
  errors.reserve(depthmap.cols * depthmap.rows);
  ...
  output_stream.open(output_filename.c_str(), std::ios::out | std::ios::binary | std::ios::trunc);
  output_stream.write(
    (const char *)error_map.data, error_map.rows * error_map.cols * sizeof(float));
  output_stream.close();
...
void MatcherTest::testWarpAffine()
{
  ...
  cv::Mat ref_patch(
    matcher.halfpatch_size_ + 1, matcher.halfpatch_size_ + 1, CV_8U, matcher.patch_with_border_);
  ...
  svo::warp::warpAffine(
    A_cur_ref, ref_ftr_->frame->img_pyr_[ref_ftr_->level], ref_ftr_->px, ref_ftr_->level, level_cur,
    matcher.halfpatch_size_ + 1, matcher.patch_with_border_);
  ...
  frame_cur_->img_pyr_
    .at(level_cur)(
      cv::Range(pxi[1] - halfpatch_size, pxi[1] + halfpatch_size),
      cv::Range(pxi[0] - halfpatch_size, pxi[0] + halfpatch_size))
    .copyTo(cur_patch);

  // display results
  cv::circle(
    frame_cur_->img_pyr_[level_cur], cv::Point2f(pxi[0], pxi[1]), patch_size, cv::Scalar(255), 2);
  cv::circle(
    frame_ref_->img_pyr_[ref_ftr_->level],
