...
: thresh(_thresh_in_px), error_multiplier2(_error_multiplier2), fts_c1(_fts1), fts_c2(_fts2)
{
}
...
void Homography::calcFromPlaneParams(const Vector3d & n_c1, const Vector3d & xyz_c1)
{
  ...
  H_c2_from_c1 =
    T_c2_from_c1.rotationMatrix() + (T_c2_from_c1.translation() * n_c1.transpose()) / d;
}
...
bool Homography::decompose()
{
  ...
  double d1 =
    fabs(singular_values[0]);  // The paper suggests the square of these (e.g. the evalues of AAT)
  ...
  if (nCase != 1) {
    printf(
      "FATAL Homography Initialization: This motion case is not implemented or is degenerate. Try "
      "again. ");
    return false;
  }
  ...
  for (size_t i = 0; i < decompositions.size(); i++) {
    ...
    for (size_t m = 0; m < fts_c1.size(); m++) {
      ...
      double dVisibilityTest =
        (H_c2_from_c1(2, 0) * v2[0] + H_c2_from_c1(2, 1) * v2[1] + H_c2_from_c1(2, 2)) / decom.d;
