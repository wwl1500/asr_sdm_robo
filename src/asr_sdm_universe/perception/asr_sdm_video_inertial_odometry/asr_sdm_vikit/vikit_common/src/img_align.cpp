...
ForwardCompositionalSE3::ForwardCompositionalSE3(
  vector<PinholeCamera> & cam_pyr, vector<cv::Mat> & depth_pyr, vector<cv::Mat> & img_pyr,
  vector<cv::Mat> & tpl_pyr, vector<cv::Mat> & img_pyr_dx, vector<cv::Mat> & img_pyr_dy,
  Sophus::SE3d & init_model, int n_levels, int n_iter, float res_thresh, bool display,
  Method method, int test_id)
: cam_pyr_(cam_pyr),
...
ForwardCompositionalSE3::ForwardCompositionalSE3(
  vector<PinholeCamera> & cam_pyr, vector<cv::Mat> & depth_pyr, vector<cv::Mat> & img_pyr,
  vector<cv::Mat> & tpl_pyr, vector<cv::Mat> & img_pyr_dx, vector<cv::Mat> & img_pyr_dy,
  int n_levels, int n_iter, float res_thresh, bool display, Method method, int test_id)
: cam_pyr_(cam_pyr),
  depth_pyr_(depth_pyr),
  img_pyr_(img_pyr),
  tpl_pyr_(tpl_pyr),
...
void ForwardCompositionalSE3::runOptimization(Sophus::SE3d & model, int levelBegin, int levelEnd)
{
  ...
  for (level_ = levelBegin; level_ >= levelEnd; --level_) {
    mu_ = 0.1;
    cout << endl << "PYRAMID LEVEL " << level_ << endl << "---------------" << endl;
    optimize(model);
  }
}

double ForwardCompositionalSE3::computeResiduals(
  const Sophus::SE3d & model, bool linearize_system, bool compute_weight_scale)
{
  // Warp the image such that it aligns with the template image
  double chi2 = 0;
  size_t n_pixels = 0;

  if (linearize_system) resimg_ = cv::Mat(tpl_pyr_[level_].size(), CV_32F, cv::Scalar(1));
...
SecondOrderMinimisationSE3::SecondOrderMinimisationSE3(
  vector<PinholeCamera> & cam_pyr, vector<cv::Mat> & depth_pyr, vector<cv::Mat> & img_pyr,
  vector<cv::Mat> & tpl_pyr, vector<cv::Mat> & img_pyr_dx, vector<cv::Mat> & img_pyr_dy,
  vector<cv::Mat> & tpl_pyr_dx, vector<cv::Mat> & tpl_pyr_dy, Sophus::SE3d & init_model,
  int n_levels, int n_iter, float res_thresh, bool display, Method method, int test_id)
: cam_pyr_(cam_pyr),
  depth_pyr_(depth_pyr),
  img_pyr_(img_pyr),
...
double SecondOrderMinimisationSE3::computeResiduals(
  const Sophus::SE3d & model, bool linearize_system, bool compute_weight_scale)
{
  // Warp the image such that it aligns with the template image
  double chi2 = 0;
  ...
  for (int v = 0; v < depth_pyr_[level_].rows; ++v) {
    for (int u = 0; u < depth_pyr_[level_].cols; ++u) {
      ...
      if (cam_pyr_[level_].isInFrame(uv_img_pyr.cast<int>(), 1)) {
        ...
        if (cam_pyr_[level_].isInFrame(uv_img_pyr.cast<int>(), 2)) mask.at<bool>(v, u) = true;
      }
    }
  }
