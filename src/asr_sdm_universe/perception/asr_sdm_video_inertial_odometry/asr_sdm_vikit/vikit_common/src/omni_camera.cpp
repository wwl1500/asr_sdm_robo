...
OmniCamera::OmniCamera(string calibFile)
{
  ...
  for (i = 0; i < *length_pol; i++) result = fscanf(f, " %lf", &pol[i]);
  ...
  for (i = 0; i < *length_invpol; i++) result = fscanf(f, " %lf", &invpol[i]);
...
Vector3d OmniCamera::cam2world(const double & u, const double & v) const
{
  ...
  // Important: we exchange x and y since regular pinhole model is working with x along the columns
  // and y along the rows Davide's framework is doing exactly the opposite
  ...
  double r =
    sqrt(pow(xyz[0], 2) + pow(xyz[1], 2));  // distance [pixels] of  the point from the image center
...
Vector2d OmniCamera::world2cam(const Vector3d & xyz_c) const
{
  ...
  // Important: we exchange x and y since Pirmin's stuff is working with x along the columns and y
  // along the rows, Davide's framework is doing exactly the opposite
  double rho;
  double t_i;
  double x;
  double y;
