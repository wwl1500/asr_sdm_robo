...
#ifdef __SSE2__
  if (
    aligned_mem::is_aligned16(in.data) && aligned_mem::is_aligned16(out.data) &&
    ((in.cols % 16) == 0)) {
...
void calcSharrDeriv(const cv::Mat & src, cv::Mat & dst)
{
  ...
  for (y = 0; y < rows; y++) {
    ...
    for (; x <= colsn - 8; x += 8) {
      ...
      __m128i t0 =
        _mm_add_epi16(_mm_mullo_epi16(_mm_add_epi16(s0, s2), c3), _mm_mullo_epi16(s1, c10));
    ...
    for (; x <= colsn - 8; x += 8) {
      ...
      __m128i t1 =
        _mm_add_epi16(_mm_mullo_epi16(_mm_add_epi16(s2, s4), c3), _mm_mullo_epi16(s3, c10));
