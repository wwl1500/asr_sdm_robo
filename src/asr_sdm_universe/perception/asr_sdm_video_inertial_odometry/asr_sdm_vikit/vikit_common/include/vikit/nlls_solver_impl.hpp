...
void vk::NLLSSolver<D, T>::optimizeGaussNewton(ModelType & model)
{
  ...
  if (use_weights_) computeResiduals(model, false, true);
  ...
  for (iter_ = 0; iter_ < n_iter_; ++iter_) {
    ...
    // add prior
    if (have_prior_) applyPrior(model);
    ...
    if ((iter_ > 0 && new_chi2 > chi2_) || stop_) {
      if (verbose_) {
        std::cout << "It. " << iter_ << "\t Failure"
                  << "\t new_chi2 = " << new_chi2 << "\t Error increased. Stop optimizing."
                  << std::endl;
      }
...
void vk::NLLSSolver<D, T>::optimizeLevenbergMarquardt(ModelType & model)
{
  ...
  if (verbose_) cout << "init chi2 = " << chi2_ << "\t n_meas = " << n_meas_ << endl;
  ...
  if (mu_ < 0) {
    double H_max_diag = 0;
    double tau = 1e-4;
    for (size_t j = 0; j < D; ++j) H_max_diag = max(H_max_diag, fabs(H_(j, j)));
    mu_ = tau * H_max_diag;
  }
  ...
  for (iter_ = 0; iter_ < n_iter_; ++iter_) {
    ...
    do {
      ...
      if (rho_ > 0) {
        ...
        nu_ = 2.;
        if (verbose_) {
          cout << "It. " << iter_ << "\t Trial " << n_trials_ << "\t Success"
        ...
        nu_ *= 2.;
        ++n_trials_;
        if (n_trials_ >= n_trials_max_) stop_ = true;

        if (verbose_) {
          cout << "It. " << iter_ << "\t Trial " << n_trials_ << "\t Failure"
               << "\t n_meas = " << n_meas_ << "\t new_chi2 = " << new_chi2 << "\t mu = " << mu_
               << "\t nu = " << nu_ << endl;
        }
      }
...
void vk::NLLSSolver<D, T>::setRobustCostFunction(
  ScaleEstimatorType scale_estimator, WeightFunctionType weight_function)
{
  switch (scale_estimator) {
    case TDistScale:
      if (verbose_) printf("Using TDistribution Scale Estimator\n");
      scale_estimator_.reset(new robust_cost::TDistributionScaleEstimator());
      use_weights_ = true;
      break;
    case MADScale:
      if (verbose_) printf("Using MAD Scale Estimator\n");
      scale_estimator_.reset(new robust_cost::MADScaleEstimator());
      use_weights_ = true;
      break;
    case NormalScale:
      if (verbose_) printf("Using Normal Scale Estimator\n");
      scale_estimator_.reset(new robust_cost::NormalDistributionScaleEstimator());
      use_weights_ = true;
      break;
    default:
      if (verbose_) printf("Using Unit Scale Estimator\n");
  ...
  switch (weight_function) {
    case TDistWeight:
      if (verbose_) printf("Using TDistribution Weight Function\n");
      weight_function_.reset(new robust_cost::TDistributionWeightFunction());
      break;
    case TukeyWeight:
      if (verbose_) printf("Using Tukey Weight Function\n");
      weight_function_.reset(new robust_cost::TukeyWeightFunction());
      break;
    case HuberWeight:
      if (verbose_) printf("Using Huber Weight Function\n");
      weight_function_.reset(new robust_cost::HuberWeightFunction());
      break;
    default:
      if (verbose_) printf("Using Unit Weight Function\n");
