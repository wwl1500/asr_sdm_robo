...
void loadBlenderDepthmap(const std::string file_name, const vk::AbstractCamera & cam, cv::Mat & img)
{
...
bool getDepthmapNormalAtPoint(
  const Vector2i & px, const cv::Mat & depth, const int halfpatch_size,
...
{
  out << gt.timestamp_ << " " << gt.image_name_ << " " << gt.t_.x() << " " << gt.t_.y() << " "
      << gt.t_.z() << " " << gt.q_.x() << " " << gt.q_.y() << " " << gt.q_.z() << " " << gt.q_.w()
      << " " << std::endl;
  return out;
}
