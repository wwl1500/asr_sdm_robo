...
bool depthFromTriangulationExact(
  const Matrix3d & R_r_c, const Vector3d & t_r_c, const Vector3d & f_r, const Vector3d & f_c,
...
double computeInliers(
  const vector<Vector3d> & features1, const vector<Vector3d> & features2, const Matrix3d & R,
  const Vector3d & t, const double reproj_thresh, double error_multiplier2,
  vector<Vector3d> & xyz_vec, vector<int> & inliers, vector<int> & outliers);

void computeInliersOneView(
  const vector<Vector3d> & feature_sphere_vec, const vector<Vector3d> & xyz_vec, const Matrix3d & R,
  const Vector3d & t, const double reproj_thresh, const double error_multiplier2,
...
inline void frameJac_xyz2uv(
  const Vector3d & xyz, const double & focal_length, Eigen::Matrix<double, 2, 6> & frame_jac)
{
