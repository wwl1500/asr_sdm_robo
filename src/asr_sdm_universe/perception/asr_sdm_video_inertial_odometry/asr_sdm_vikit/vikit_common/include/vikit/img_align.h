...
public:
  ...
  ForwardCompositionalSE3(
    ...
    Sophus::SE3d & init_model, int n_levels, int n_iter = 50, float res_thresh = 0.2,
    bool display = true, Method method = LevenbergMarquardt, int test_id = 0);

  ForwardCompositionalSE3(
    vector<PinholeCamera> & cam_pyr, vector<cv::Mat> & depth_pyr, vector<cv::Mat> & img_pyr,
    vector<cv::Mat> & tpl_pyr, vector<cv::Mat> & img_pyr_dx, vector<cv::Mat> & img_pyr_dy,
    int n_levels, int n_iter = 50, float res_thresh = 0.2, bool display = true,
    Method method = LevenbergMarquardt, int test_id = 0);

  void runOptimization(Sophus::SE3d & model, int levelBegin = -1, int levelEnd = -1);
};
