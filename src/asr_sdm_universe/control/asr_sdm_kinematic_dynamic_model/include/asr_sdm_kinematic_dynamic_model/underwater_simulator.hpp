...
struct UnderwaterSimulatorState
{
  ...
  /// Visualization-only accumulated spin angle of each propeller, in radians. Positive
  /// follows the rotor's handedness convention (left-hand rotor spins negative, right-hand
  /// positive). Updated purely for joint-state publishing so RViz can see the propellers
  /// turning when a thrust command is applied; this state never feeds back into the dynamics.
  RotorVector rotor_position = RotorVector::Zero();
};
...
enum class IntegrationMethod
{
  ...
  RungeKutta4,
  /// Fully implicit Euler. Unconditionally stable for stiff systems; converges in a few
  /// Newton iterations (default 5) per step. Use this when large linear_damping or
  /// added-mass coefficients cause the explicit methods to diverge.
  ImplicitEuler,
};
...
struct UnderwaterSimulatorParameters
{
  ...
  /// Maximum Newton iterations per step when integration_method is ImplicitEuler.
