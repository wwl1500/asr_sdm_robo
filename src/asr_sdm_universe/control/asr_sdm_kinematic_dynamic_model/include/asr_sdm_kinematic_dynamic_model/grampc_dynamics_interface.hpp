...
struct GrampcCostWeights
{
  ...
  /// Logarithmic barrier weight on the quaternion part of the configuration state.
  /// Setting this to a positive value discourages large orientation deviations.
  double quaternion_log_weight{0.0};

  bool isValid() const
  {
    return state_weight.allFinite() && control_weight.allFinite() &&
           terminal_weight.allFinite() && std::isfinite(quaternion_log_weight) &&
           !(state_weight.array() < 0.0).any() &&
           !(control_weight.array() < 0.0).any() &&
           !(terminal_weight.array() < 0.0).any() &&
           quaternion_log_weight >= 0.0;
  }
};
...
public:
  ...
  /// Enable velocity limits as inequality constraints on the MPC state [delta_q, v].
  /// Linear velocity, angular velocity, and joint velocity bounds are enforced by
  /// four inequality constraints: h(x) = |v_linear| - max_linear <= 0, etc.
  void setVelocityLimits(double max_linear, double max_angular, double max_joint);
  void disableVelocityLimits();
  bool hasVelocityLimits() const {return velocity_limits_enabled_;}

  /// Disables the state-dimension and finite-value checks in validateStateAndControl().
  /// Call this before wrapping this object inside StochasticGrampcDynamics, because
  /// the sigma-point wrappers slice the N×nx expanded state into single nx chunks
  /// that would otherwise fail those checks.
  void enableRelaxedValidationForStochasticCallbacks()
  {
    relaxed_validation_ = true;
  }

  /// Returns true if the quaternion (indices 3–6) in configuration is finite and
  /// normalized to 1.0, and all other entries are finite.
...
private:
  ...
  // Velocity limit constraints (inequality constraints)
  bool velocity_limits_enabled_{false};
