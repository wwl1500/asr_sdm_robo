...
int main()
{
  ...
  // 使用流体密度 = 淡水 (1000 kg/m³)
  params.hydrodynamics.fluid_density = 1000.0;

  // 设置流体动力学参数
  // URDF 总质量约 1.0 kg (4 段 + joint_units)
  // 中性浮力：rho * V_total = m_total，需要 V_per_link ≈ 0.25 L
  // 使用稍大体积以确保浮力主导（抵消重力）
  const double displaced_volume = 0.0005;  // 0.5L per link，2L 总 = 2 kg 排水
  for (auto & link : params.hydrodynamics.links) {
    link.mass = 0.4;
    link.displaced_volume = displaced_volume;
    link.added_mass.setIdentity();
    link.added_mass *= 0.1;
    link.linear_damping.setConstant(0.05);
