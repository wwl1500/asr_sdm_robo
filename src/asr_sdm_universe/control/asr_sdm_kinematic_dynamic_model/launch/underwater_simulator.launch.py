...
def generate_launch_description():
    ...
    # Path to the Python panel script installed by CMakeLists.txt.
    panel_script = (
    ...
    simulator_node = Node(
        ...
        parameters=[{
            ...
            'joint_viscous_damping': LaunchConfiguration('joint_viscous_damping'),
            'joint_coulomb_friction': LaunchConfiguration('joint_coulomb_friction'),
    ...
    # ExecuteProcess wrapping a bare python3 (no bash trap).  Launch sends SIGTERM
    # to the entire process group, so the python3 panel receives it directly.
    # The panel uses signal.set_wakeup_fd() + QSocketNotifier to convert the
    # signal into a Qt event that runs inside app.exec_(), ensuring clean exit.
    control_panel = ExecuteProcess(
        cmd=['python3', str(panel_script), '--node', '/underwater_simulator_node'],
        output='screen',
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('with_control_panel', default_value='true'),
