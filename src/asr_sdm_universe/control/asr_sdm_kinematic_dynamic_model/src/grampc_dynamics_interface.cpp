  ...
  const char * callback) const
{
  ...
  if (relaxed_validation_) {
    return;  // Skipped for stochastic wrappers that slice the expanded state
  }
  ...
  const double quat_norm = x.head<4>().norm();
  if (quat_norm < 1.0e-12 || std::abs(quat_norm - 1.0) > 1.0e-5) {
    throw std::invalid_argument(
      std::string(callback) + ": quaternion in x must be finite and normalized");
  }
  if (!u.allFinite()) {
  ...
  const MpcState & state, const Eigen::Matrix<double, kMpcControlDim, 1> & control) const
{
  ...
  MpcState derivative;
  // In the tangent space around referenceConfiguration(), integrate() acts linearly on
  // delta_q, so its derivative with respect to delta_q is the identity matrix.
  // Therefore d(integrate)/dt = I * velocity = velocity.
  derivative.head<kMpcConfigurationTangentDim>() = velocity;
...
void GrampcUnderwaterDynamics::setVelocityLimits(
  double max_linear, double max_angular, double max_joint)
{
  if (max_linear <= 0.0 || max_angular <= 0.0 || max_joint <= 0.0) {
  ...
  max_joint_velocity_ = max_joint;
  velocity_limits_enabled_ = true;
}

void GrampcUnderwaterDynamics::disableVelocityLimits()
{
  velocity_limits_enabled_ = false;
}
