...
private:
  ...
  UnderwaterSimulatorParameters makeSimulatorParameters()
  {
    ...
    if (integrator == "semi_implicit_euler") {
      ...
      params.integration_method = IntegrationMethod::ImplicitEuler;
    } else {
      throw std::runtime_error(
        "integrator must be 'semi_implicit_euler', 'rk4', or 'implicit_euler'");
    }

    params.max_newton_iterations =
      declare_parameter<int>("max_newton_iterations", 5);
    ...
    const UnderwaterSimulatorState & state, const UnderwaterDynamicsOutput & output)
  {
    ...
    for (std::size_t rotor = 0; rotor < kNumRotors; ++rotor) {
      joint_state.name.push_back(rotor_joint_names_[rotor]);
      joint_state.position.push_back(state.rotor_position(static_cast<Eigen::Index>(rotor)));
