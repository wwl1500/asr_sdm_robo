...
: params_(params), screw_model_(params.screw)
{
  ...
  if (!params_.rotor_force_axis_world.array().isFinite().all() ||
    params_.rotor_force_axis_world.norm() < 1.0e-12)
  {
  ...
  params_.rotor_force_axis_world.normalize();
}
  ...
  const JointTorqueVector & joint_torque) const
{
  ...
  for (std::size_t link = 0; link < kNumLinks; ++link) {
    for (std::size_t rotor = 0; rotor < 2; ++rotor) {
      ...
      // Convert world-frame thrust axis to rotor-local frame for the wrench.
      // rotor_force_axis_world is the desired thrust direction in world coordinates.
      // R_local_world transforms vectors from local frame to world frame.
      // So R_world_local = R_local_world^T transforms world vectors to local frame.
      const auto & R_local_world = kinematics.rotor_placements[link][rotor].rotation();
      const Eigen::Vector3d rotor_force_axis_local = R_local_world.transpose() * params_.rotor_force_axis_world;

      SpatialVector wrench = SpatialVector::Zero();
      wrench.head<3>() = thrust * rotor_force_axis_local;
      if (params_.enable_reaction_torque) {
        ...
        wrench.tail<3>() = output.rotor_reaction_torque(index) * rotor_force_axis_local;
      }
