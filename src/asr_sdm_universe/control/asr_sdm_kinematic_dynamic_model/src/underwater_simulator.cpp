...
: gravity_world_(params.gravity_world),
  ...
  max_newton_iterations_(params.max_newton_iterations),
  ...
  const UnderwaterSimulatorInput & input) const
{
  ...
  const auto guard = [](const ReducedVelocity & increment) -> const ReducedVelocity & {
      ...
      if (!increment.array().isFinite().all() || increment.norm() > kMaxIncrementNorm) {
        throw std::runtime_error(
          ...
          "IntegrationMethod::SemiImplicitEuler or ::ImplicitEuler");
      }
      return increment;
    };
  ...
  } else if (integration_method_ == IntegrationMethod::ImplicitEuler) {
    // Fixed-point (Picard) iteration for the implicit Euler equation:
    //   v_{n+1} = v_n + dt * f(q_n, v_{n+1})
    //   q_{n+1} = integrate(q_n, dt * v_{n+1})
    // Each iteration re-evaluates the dynamics at the tentative new velocity, so this
    // loop converges in a few steps for most stiff underwater systems.
    ReducedVelocity trial_velocity = state.velocity + dt * output.acceleration;
    for (int iter = 0; iter < max_newton_iterations_; ++iter) {
      UnderwaterSimulatorState probe = state;
      probe.velocity = trial_velocity;
      const ReducedAcceleration acc =
        accelerationOnly(probe, input);
      trial_velocity = state.velocity + dt * acc;
      // Clamp velocity norm to prevent divergence during iteration.
      constexpr double kMaxTrialVelocity = 1.0e6;
      if (trial_velocity.norm() > kMaxTrialVelocity) {
        trial_velocity = trial_velocity.normalized() * kMaxTrialVelocity;
      }
    }
    next_velocity = trial_velocity;
    const ReducedVelocity increment = guard(dt * next_velocity);
    next_configuration = pinocchio_model_.integrate(state.configuration, increment);
  } else {
  ...
  // Visualization-only: spin the propellers at a rate proportional to their thrust.
  // We don't have a closed-form rotor speed from the actuator model in
  // AggregateThrust mode, so we map thrust -> spin by a fixed gain. The sign comes
  // from each screw's handedness so left/right rotors spin in opposite directions.
  constexpr double kVisualizationSpinGain = 30.0;  // rad/s per Newton of thrust
  RotorVector next_rotor_position = state.rotor_position;
  const auto & screw = actuator_model_.screwDriveModel();
  for (std::size_t i = 0; i < kNumRotors; ++i) {
    const double handedness = screw.handedness(i);
    // Constrain to keep the spin reasonable even if thrust is large.
    const double spin_rate = std::clamp(
      handedness * next_rotor_thrust(static_cast<Eigen::Index>(i)) * kVisualizationSpinGain,
      -50.0, 50.0);
    next_rotor_position(static_cast<Eigen::Index>(i)) += dt * spin_rate;
  }

  if (!next_configuration.array().isFinite().all() || !next_velocity.array().isFinite().all() ||
    !next_rotor_thrust.array().isFinite().all() || !next_rotor_position.array().isFinite().all())
  {
  ...
  state.rotor_position = next_rotor_position;
  state.time += dt;
  return output;
}
