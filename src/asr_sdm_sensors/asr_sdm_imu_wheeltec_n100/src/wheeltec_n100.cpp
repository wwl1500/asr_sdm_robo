#include "asr_sdm_imu_wheeltec_n100/wheeltec_n100.hpp"
...
public:
  ImuNode() : Node("imu_node")
  {
    ...
    imu_trueEast_pub_ =
      this->create_publisher<sensor_msgs::msg::Imu>(imu_trueEast_topic_.c_str(), 10);
    mag_pose_pub_ =
      this->create_publisher<geometry_msgs::msg::Pose2D>(mag_pose_2d_topic_.c_str(), 10);
...
private:
  ...
  void read_imu()
  {
    ...
    while (rclcpp::ok()) {
      ...
      if (
        head_type[0] != TYPE_IMU && head_type[0] != TYPE_AHRS && head_type[0] != TYPE_INSGPS &&
      ...
      if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN) {
        ...
        continue;
      } else if (head_type[0] == TYPE_GROUND || head_type[0] == 0x50)  // Unknown data, prevent
                                                                       // record failure
      {
        uint8_t ground_sn[1];
      ...
      if (head_type[0] == TYPE_AHRS) {
        ...
        } else if (device_type_ == 1)  // Coordinate transformation under ROS standard of imu single
                                       // product
        {
        ...
        } else if (device_type_ == 1) {  // Coordinate transformation of car and imu single product
                                         // ROS standard//
          magx = imu_frame_.frame.data.data_pack.magnetometer_x;
          magy = imu_frame_.frame.data.data_pack.magnetometer_y;
          magz = imu_frame_.frame.data.data_pack.magnetometer_z;
        ...
        std::fill(
          mag.magnetic_field_covariance.begin(), mag.magnetic_field_covariance.end(),
          mag_covariance_);
